# Radar: investigación previa de los ocho candidatos

Fecha de la investigación: **2026-09-16** (WebSearch; todo lo volátil caduca a los seis meses). Es el
material de partida para escribir cada ficha con `docs/TONO.md`. Lo que no se pudo verificar lleva
**(sin verificar)** y debe entrar en la ficha con `confidence: "baja"`.

Convención: BCN = Barcelona-El Prat salvo que se diga Girona. "DNI" = los españoles entran con el DNI.

---

## Kirguistán (kg · 417) — prioridad alta

**Entrada.** Sin visado, pero **desde el 31/12/2025 solo 30 días dentro de cualquier periodo de 60**
(Resolución 855; antes eran 60 en 120). Pasaporte, no DNI. Fuentes viejas siguen diciendo 60: no fiarse.

**Vuelos.** Sin directo. Pegasus por Estambul (Sabiha) y Turkish por Estambul, ≈ 9-10 h; ida y vuelta
desde ≈ 650 €. Aterrizaje en Manas (Biskek) de madrugada.

**Transporte.**
- Sin buses grandes Biskek–Osh: marshrutka o taxi compartido 10-12 h, o vuelo de 45 min. La línea de
  tren del sur está partida por Uzbekistán y no funciona desde 2010.
- **Tren de verano Biskek-2 → Balykchy (Issyk-Kul):** del 5 al 21 de junio viernes-domingo, del 26 de
  junio al 13 de septiembre diario; sale 08:06, llega 12:12 (12:37 en "Balykchy Playa"); 14 vagones,
  billetes desde 150 som (≈ 1,5 €). Es el único tren útil del país y es un sitio circo en sí.
- Marshrutkas Biskek–Karakol (6-7 h), Biskek–Kochkor (3 h). Song-Kul: desde Kochkor con CBT
  (yurtas, 60-80 $ por persona con comidas en las "premium"; reservar 1-2 semanas en julio-agosto).
- Bolt/Yandex Go en Biskek.

**Sitios circo verificados.**
- **Circo de Biskek** (1976, platillo volante, en uso): **cerrado por reforma desde julio de 2024** y la
  fachada ha perdido los motivos originales. Decirlo. Resto del brutalismo de Frunze: Museo Histórico,
  Palacio de Deportes, Filarmónica, la Casa Blanca; hay rutas autoguiadas publicadas.
- **Mailuu-Suu**: antigua ciudad del uranio soviético (550 km de Biskek, provincia de Jalal-Abad), uno de
  los siete sitios prioritarios del OIEA por los residuos; en 2022 abrió un **Museo de la Historia de
  Mailuu-Suu y del Patrimonio del Uranio**. Se llega desde Jalal-Abad en marshrutka (sin verificar
  horarios). Remediación en curso con dinero de la UE y China.
- **Juegos Nómadas Mundiales 2026**: ya celebrados, del 31 de agosto al 6 de septiembre de 2026 en
  Biskek (ceremonias) y Cholpon-Ata (competición: kok-boru, er enish, águilas). Los siguientes serán en
  2028 (sede por decidir). Para la ficha, el kok-boru se ve en los hipódromos en Nooruz (21 de marzo) y en
  fiestas locales de verano.
- Tash Rabat (caravasar del XV en la carretera de Torugart), Karakol con su iglesia de madera y mezquita
  dungana, el cañón de Skazka, el Issyk-Kul con los sanatorios soviéticos de Cholpon-Ata (sin verificar
  estado).

**Riesgos de datos.** Mailuu-Suu: transporte y qué se puede ver sin escolta (sin verificar). Circo:
puede reabrir en cualquier momento.

---

## Turquía (tr · 792) — prioridad alta

**Entrada.** **Con el DNI vale**: el Ministerio de Exteriores turco lista a España entre los países cuyos
ciudadanos entran con el documento nacional de identidad. Sin visado, 90 días en 180.

**Vuelos.** Directos a diario: Turkish (≈ 25/semana a IST), Pegasus (≈ 10/semana a SAW), Vueling
(≈ 7/semana). 3 h 30. Desde 80-110 € por trayecto.

**Transporte.**
- **Doğu Ekspresi (Ankara–Kars)**: diario, sale de Ankara 17:55 y llega a Kars 20:27 del día siguiente
  (26 h); vuelta 08:00 → 09:56. Asientos y literas; **se agota en minutos** en tcdd.gov.tr. El
  **Turístico** (solo coches cama, con paradas de 3 h en Erzincan, Erzurum, etc.) circula de diciembre a
  marzo, con precios de tour. Ojo con los dos nombres.
- **YHT Estambul–Ankara**: ≈ 15 al día, 4-4,5 h, desde ≈ 9-25 € según fuente (usar 25 € para 2026,
  `media`).
- **Kars → Ani**: un minibús diario a las 10:00 (desde Resil Yıldız / Gazi Ahmet Paşa) por ≈ 20 TL, o
  taxi con espera de 3 h por 150-200 TL negociados; la oficina de turismo de Kars los monta.
- **Kayaköy** (pueblo griego fantasma, 1923): dolmuş desde Fethiye cada 30 min en verano (cada hora en
  invierno), 20-25 min; entrada 3 € (mayo 2026).
- Derinkuyu (ciudad subterránea de 85 m, 20.000 personas): abre 8:00-18:00/19:00 verano; desde Nevşehir
  en bus.

**Sitios circo.** Ani (la capital armenia fantasma en la frontera cerrada), Kars con su arquitectura
rusa, Kayaköy, Derinkuyu/Kaymaklı, el Doğu Ekspresi como sitio en sí, Ankara con Anıtkabir. Pendiente de
investigar: Diyarbakır (murallas y Sur, tras 2016), Şanlıurfa/Göbekli Tepe, la base de Hasankeyf
inundada (el pueblo viejo quedó bajo el embalse de Ilısu en 2020: sitio de catástrofe).

**Riesgos.** Este de Turquía: MAEC recomienda precaución en las provincias fronterizas con Siria e Irak;
Kars y Ani no están en esa zona pero comprobar. El Doğu se compra con exactamente 15 días de antelación
a medianoche hora turca (sin verificar el plazo exacto de 2026).

---

## Polonia (pl · 616) — prioridad alta

**Entrada.** DNI (UE, Schengen). Złoty, no euro (≈ 4,3 por euro). Roaming UE.

**Vuelos.** Cracovia: Ryanair, Wizz y Buzz, ≈ 27/semana, desde 23 €. Varsovia Chopin: LOT, Wizz,
≈ 36/semana; Ryanair a Modlin ≈ 6/semana. 3 h.

**Transporte.**
- **Nowa Huta**: tranvía 4 (también 10) desde Teatr Słowackiego o Dworzec Główny Tunel hasta Plac
  Centralny, ≈ 30 min, y sigue hasta la puerta de la acería. Aviso: en 2026 hubo obras/avería en las
  vías de Nowa Huta con cortes en Plac Centralny; comprobar.
- **Guarida del Lobo (Gierłoż)**: tren a Kętrzyn (desde Olsztyn) y bus local desde Sezam/Sikorskiego a
  "Gierłoż Kwatera", 4 al día de lunes a sábado, 13 min. Entrada ≈ 40 zł (sin verificar 2026).
- **Treblinka**: tren Warszawa Gdańska → Małkinia (≈ 1 h 10, ≈ 9 zł, pocos al día) y desde allí 8 km:
  taxi o a pie; **la vuelta es el problema** (pocos trenes). El museo tiene página de "transports" con
  las conexiones.
- **Palacio de la Cultura**: terraza del piso 30 (114 m), 25 zł.
- **Podborsko 3001** (búnker soviético de cabezas nucleares "Monolit", Pomerania): museo con visitas de
  mayo a octubre a las 11, 12 y 13, grupos de 40; cerca de Borne Sulinowo (la ciudad militar soviética
  secreta, hoy habitada). Sin tren directo: bus a Białogard/Tychowo y taxi (sin verificar).

**Sitios circo.** Nowa Huta (Plac Centralny, la acería Lenin, la iglesia Arka Pana, el tanque), Auschwitz
(bus desde Cracovia, reserva online obligatoria), Treblinka, Palacio de la Cultura, Guarida del Lobo,
Podborsko y Borne Sulinowo, el búnker de Konewka, el gueto de Varsovia y el museo POLIN, Sobibór y
Bełżec (trenes lentos), Wieliczka (mina de sal con catedral), el tren de Hel. Pendiente: Legnica ("la
pequeña Moscú" soviética), Łódź industrial.

**Riesgos.** Auschwitz exige reserva con semanas en temporada. Treblinka sin coche es un día entero.

---

## Hungría (hu · 348) — prioridad media

**Entrada.** DNI. Forinto (≈ 400 por euro). Roaming UE.

**Vuelos.** Wizz, Ryanair y Buzz directos, ≈ 19/semana, desde 25-35 €. 2 h 30.

**Transporte.**
- **Memento Park**: bus 101B/101E desde Kelenföld (metro M4), abre 10:00 hasta el anochecer.
- **Hospital en la Roca** (búnker nuclear bajo el castillo de Buda): solo con visita guiada de 60 min;
  bus 16 desde Deák.
- **Budapest–Pécs**: 7 trenes directos al día, ≈ 3 h 40, desde ≈ 10 €.
- MÁV vende online; Bolt en Budapest.

**Sitios circo.**
- **Szentkirályszabadja** (junto a Veszprém y el Balatón): la ciudad militar soviética abandonada más
  grande de Hungría (escuelas, teatro, bloques); acceso libre (sin verificar estado 2026).
- **Club Aliga (Balatonaliga)**: el complejo vacacional del Comité Central en el Balatón, abandonado.
- **Recsk**: campo de trabajos forzados de la ÁVH (1950-53, 1.500 presos); parque memorial nacional a
  6 km del pueblo, en el bosque; bus a Recsk desde Eger (sin verificar).
- Pécs: la mina de uranio de Mecsek (1956-97) solo en el Museo de la Minería (bodegas); la mina en sí,
  cerrada.
- Budapest: Casa del Terror, Memento Park, Hospital en la Roca, el Parlamento con la estrella que
  quitaron, los baños, el metro M1, Kőbánya (bodegas-búnker de cerveza, visitas puntuales).

**Riesgos.** Balatón sin coche: tren a Balatonaliga y Veszprém, y a pie; comprobar accesos.

---

## Eslovaquia (sk · 703) — prioridad media

**Entrada.** DNI. Euro. Roaming UE.

**Vuelos.** Bratislava: Ryanair y Wizz, ≈ 16/semana (Ryanair lo mantiene en verano e invierno 2026-27),
desde ≈ 35 €. Alternativa: Viena (Vueling, Ryanair) y bus de 1 h.

**Transporte.**
- **Trenes gratis** (ZSSK, trenes Os y REX) para **menores de 26 estudiantes y mayores de 62 de la UE**,
  con registro en taquilla y tarjeta de cliente; los mayores de 62 solo con documento. Para el resto,
  baratos (Bratislava–Košice ≈ 20 €). Nuestro perfil no entra, pero merece una línea.
- **UFO (Most SNP)**: mirador 7,40-11 € según día; gratis si se consume en el restaurante.
- **Banská Štiavnica**: Museo Minero al aire libre con recorrido subterráneo por la galería Bartolomej
  (niveles a 33 y 45 m, 1.300 m, 70-90 min, casco y linterna); tren con cambio en Hronská Dúbrava.

**Sitios circo.** Petržalka (el bloque más grande de Europa central, con la vista desde el UFO), la
pirámide invertida de la Radio Eslovaca, el crematorio de Bratislava, el memorial de Slavín, el búnker
B-S 8 (línea de defensa checoslovaca, museo), el paso de Devín con el Telón de Acero y el memorial de
los fusilados al cruzar, Banská Štiavnica, el memorial del Levantamiento en Banská Bystrica (el
"platillo" de 1969), los Tatras con el hotel Panorama y el funicular. Pendiente: Košice y el este,
Chatam Sofer, el sanatorio Helios en Štrbské Pleso.

---

## Macedonia del Norte (mk · 807) — prioridad media

**Entrada.** DNI (fuentes españolas de 2026 lo confirman; el MAEC dice DNI o pasaporte). 90 días.
Denar (≈ 61 por euro). Sin roaming UE.

**Vuelos.** **Wizz BCN–Skopje directo, 2-3 por semana**, 2 h 45, desde ≈ 35 €.

**Transporte.**
- **Skopje–Bitola en tren**: 4 al día, ≈ 3 h 25 (el primero a las 14:30, el último 20:05 → 23:29);
  horario 2026 publicado. Precios 5-10 €.
- **Skopje–Ohrid en bus**: 9 al día (06:30-19:30), 3-3,5 h, 950-980 MKD (≈ 16 €).
- **Kruševo (Makedonium)**: dos buses directos desde Skopje y dos desde Bitola, o vía Prilep; el
  monumento está en una colina sobre el pueblo, a pie desde el centro.

**Sitios circo.** **Skopje 2014**: en febrero de 2018 el gobierno paró el programa y empezó a retirar
monumentos; se creó una comisión para los demás (Alejandro, Filipo). En 2026 la mayoría de estatuas
**siguen en pie** (sin verificar cuántas han quitado): la ficha debe describirlo como "parado y en
retirada lenta". Brutalismo post-terremoto de 1963 (Kenzo Tange: la estación, Correos, el Telecom), el
antiguo Ayuntamiento, el Makedonium de Kruševo (1974), el cementerio de Bitola, Ohrid (UNESCO), el
monasterio de San Naum, el cañón de Matka, la mezquita pintada de Tetovo. Pendiente: la mina de
Lojane (arsénico, abandonada), el memorial de Veles.

**Riesgos.** Tensión étnica albanesa-macedonia baja pero presente; identidad nacional sensible (no
llamarlo "Macedonia" a secas en la ficha).

---

## Montenegro (me · 499) — prioridad media

**Entrada.** **DNI hasta 30 días; pasaporte hasta 90.** Euro (sin ser de la eurozona). Sin roaming UE.

**Vuelos.** **Vueling BCN–Tivat solo de julio a septiembre**; Podgorica con low-cost a Barcelona según
temporada (sin verificar operador). El resto del año: Belgrado + tren, o Dubrovnik (Vueling) + bus de
2 h a Kotor.

**Transporte.**
- El **Belgrado–Bar** entra por Bijelo Polje y para en Podgorica; Belgrado–Podgorica 21,60 € (2.ª),
  Belgrado–Bar 23,80 €. **Podgorica–Bar**: 10 trenes al día, ≈ 1 h, 2,40 €. Horarios en zpcg.me.
- Buses Podgorica–Kotor: ≈ 5 directos al día, 2-2,5 h; la costa entera en bus.

**Sitios circo.**
- **Hotel Fjord (Kotor): demolido en 2018.** No ponerlo como visitable; la ficha puede contarlo como lo
  que se perdió.
- **Mamula**: la isla-fortaleza (campo de concentración italiano en 1942) es desde 2023 un hotel de 32
  habitaciones y desde 2025 Banyan Tree; no se visita salvo como cliente (desde ≈ 900 €/noche).
- **Barutana** (monumento a los caídos de Lješanska nahija, 1980): a 10 km de Podgorica hacia Cetinje;
  bus a Cetinje bajándose en Barutana (sin verificar parada). Monumento partisano de Gorica (Podgorica,
  en el parque). Lista de espomeniks de Montenegro en Wikipedia para elegir (Nikšić, Kolašin, Berane).
- El viaducto de Mala Rijeka, Podgorica brutalista (Blok 5, el hotel Podgorica), Cetinje (la antigua
  capital con embajadas), Žabljak y Durmitor (bus), la bahía de Kotor con la carretera de las 25 curvas.

**Riesgos.** Temporada corta del directo; en julio-agosto la costa está saturada.

---

## Corea del Sur (kr · 410) — prioridad media-baja (cara y lejos)

**Entrada.** Sin visado 90 días. **K-ETA exenta para España hasta el 31/12/2026** (prórroga anual);
tarjeta de llegada digital (e-Arrival) obligatoria. Pasaporte.

**Vuelos.** **Directo BCN–Seúl Incheon: Asiana (diario desde septiembre de 2026), Korean Air, T'way**
(≈ 12/semana), 12 h 30. Desde ≈ 600-700 € ida y vuelta en buenas fechas.

**Transporte.**
- **KTX Seúl–Busan**: ≈ 2 h 15, 59.800 ₩ (≈ 40 €) en clase estándar; Mugunghwa lento 5 h 20, 8 al día,
  solo de día (**no hay nocturno**).
- Metro y T-money; Naver Map/Kakao Map (Google Maps no navega).

**Sitios circo.**
- **DMZ**: los tours normales (tercer túnel, Dora, Imjingak) funcionan con normalidad en 2026, reserva de
  2-3 semanas, pasaporte físico, 55.000-95.000 ₩. **Panmunjom/JSA**: el centro de visitantes reabrió a
  medias en verano de 2025, pero **las casetas azules siguen cerradas desde el incidente de Travis King
  (julio de 2023)** y no hay calendario fiable; 150-195.000 ₩ cuando sale. `baja`.
- **Cheorwon**: la sede del Partido de los Trabajadores norcoreano (1946, en ruinas), el 2.º túnel, el
  observatorio de la paz y la estación de Woljeong-ri, en un tour local menos masificado que Paju.
- **SeMA Bunker (Yeouido)**: el búnker secreto de 1970 hecho sala de arte **cerró el 13 de enero de 2025**
  (sin verificar si ha reabierto). No darlo por abierto.
- Gamcheon (Busan), Jeongdongjin (la estación más cerca del mar del mundo, Guinness), el búnker/museo
  de la guerra de Seúl, Seodaemun (cárcel japonesa), Gwangju y el 18 de mayo, el templo stay.

**Riesgos.** El más caro del radar en el día a día (80-100 €/día); el móvil es obligatorio para todo.

---

## Orden sugerido para escribir
1. **Polonia** y **Turquía**: DNI, directos diarios, mucho circo verificado, encajan con Bosnia/Serbia y
   con Georgia por tierra. 2. **Kirguistán**: el más raro y con el tren de verano como excusa; encadena
   con Kazajistán y Uzbekistán. 3. Hungría + Eslovaquia como pareja (Budapest–Bratislava en 2 h 30 de
   tren). 4. Macedonia + Montenegro como pareja balcánica (Skopje–Podgorica en bus). 5. Corea, cuando
   se sepa qué pasa con el JSA.

---

## Estado
Los ocho tienen ficha completa desde el 2026-09-16 (commits `21a3b24`, `2286f52`, `877eaff` y `70f3992`).
Lo marcado como `baja` aquí (JSA, SeMA Bunker, permisos) está en `meta.notes` de cada sitio. El tercer
radar (Alemania del Este, Grecia, Eslovenia, Croacia, Finlandia, Chipre) está en `src/data/candidates.ts`
sin investigar todavía.


---
---

# Tercer radar: investigación previa de los seis candidatos

Fecha de la investigación: **2026-09-17** (WebSearch; todo lo volátil caduca a los seis meses). Mismo
método y convenciones que arriba. Los seis son UE (menos la mitad norte de Chipre) y entran con el DNI.

---

## Chipre (cy · 196) — prioridad alta

**Entrada.** UE: DNI. Para cruzar la Línea Verde al norte vale el DNI de la UE (se enseña en el paso; el
cruce es gratis y tarda 15 min). Ir por el sur (Larnaca) es lo legal para la República; entrar por Ercan
(norte) y pasar al sur puede dar problemas: no hacerlo.

**Vuelos.** BCN–Larnaca directo con Vueling, Ryanair, Wizz, Aegean y SKY express (varios al día entre
todas, 4 h). **A Pafos no hay directo desde BCN** (sí desde otras ciudades españolas con Ryanair).

**Transporte.**
- Sin tren. **InterCity Buses** une todas las ciudades a diario: Nicosia–Larnaca (≈ 45 min), Nicosia–
  Limassol, Nicosia–Pafos, Larnaca–Ayia Napa; 4-9 € el billete, mínimo 3 €. Buses urbanos por ciudad
  (Pafos Buses, OSEL en Nicosia, EMEL en Limassol).
- Norte: desde Nicosia norte (Lefkoşa) bus **İtimat** a Famagusta (Gazimağusa) **cada hora de 07:00 a
  18:00**, ≈ 1 h; a Kyrenia (Girne) **VirgoBus cada 30 min**, 45-55 min. De Kyrenia a Bellapais, taxi.
  Se paga en liras turcas o euros.
- Cruces peatonales en Nicosia: **Ledra Street** (en el centro) y Ledra Palace.

**Sitios circo verificados.**
- **Varosha (Famagusta)**: el barrio fantasma cerrado desde 1974, **abierto a medias desde 2020**: se
  entra gratis, en verano 08:00-20:00 (en invierno cierra antes), por calles marcadas; **se alquilan
  bicis dentro** (tasa pequeña); prohibido salirse de los carriles y entrar en los edificios. Sigue
  siendo zona militar turca con vallas y carteles; la mayoría de bloques a la vista.
- **Nicosia**: la última capital dividida; la Línea Verde con sacos terreros y barriles en el casco
  viejo; **tours guiados de la zona tampón** (Ledra Palace, muralla veneciana, puerta de Famagusta)
  funcionando en 2026, 2-3 h. El **aeropuerto abandonado de Nicosia** está en la zona de la ONU: **no se
  visita** salvo permiso de la ONU para investigadores; hay un paseo virtual del Cyprus Institute.
  Decirlo en la ficha como "lo que no se ve".
- Kyrenia (castillo, museo del pecio de 2.300 años), Bellapais, Salamina (ruinas junto a Famagusta),
  las bases británicas (Akrotiri, Dhekelia: se cruzan en bus, no se visitan), Choirokoitia (neolítico,
  bus Nicosia–Limassol para en la carretera), el pecio Zenobia (buceo, Larnaca).

**Riesgos de datos.** Horarios de Varosha cambian sin aviso; el norte cobra en liras (volátil); en
verano 40 °C.

---

## Grecia (gr · 300) — prioridad alta

**Entrada.** UE: DNI.

**Vuelos.** BCN–Atenas directo con **Aegean (≈ 15/semana) y Vueling (≈ 14/semana)**, 3 h 15. Ryanair
solo con escala. Desde ≈ 85 € por trayecto.

**Transporte.**
- **Atenas–Tesalónica en tren (Hellenic Train)**: funciona, 4-6 al día, ≈ 5 h, 25-35 € en turista;
  pero la línea sigue en obras tras la tormenta Daniel (2023) y el accidente de Tempi: cortes de
  semanas (junio-julio de 2026), tramos en bus y **vía única en Agios Stefanos–Afidnai desde
  septiembre de 2026**. Comprobar en hellenictrain.gr antes.
- **Meteora sin tren directo**: el tramo Palaiofarsalos–Kalambaka sigue cerrado (previsto para finales
  de 2026 o 2027). Opciones: tren a Palaiofarsalos + bus de Hellenic Train (5-6 h en total) o **bus
  exprés Atenas–Kalambaka 4 h 30**. En la ficha, Meteora va en bus.
- Ferris de Pireo y Rafina a todo; metro de Atenas al Pireo y al aeropuerto; KTEL a lo demás.

**Sitios circo verificados.**
- **Spinalonga (Creta)**: la leprosería en la isla-fortaleza veneciana; barcos desde Plaka (12 € ida y
  vuelta, primer barco 10:00 y último desde Plaka a las 17:00) y desde Elounda cada 30 min;
  **entrada 20 €** (gratis para menores de 25 de la UE). Bus KTEL Heraklion–Agios Nikolaos–Elounda–Plaka.
- **Parque de la Libertad (Atenas)**: el antiguo cuartel de la policía militar de la Junta (EAT-ESA)
  con el **Museo de la Resistencia Antidictatorial** dentro de las celdas de tortura (martes-sábado
  10-13 y 17-21, domingo 10-13; gratis). Metro Megaro Mousikis.
- **Politécnico de Atenas**: la verja retorcida por el tanque del 17 de noviembre de 1973 y la cabeza
  de bronce de Markis, en Patission; en la calle, sin horario.
- **Makronisos y Gyaros** (las islas-prisión de la guerra civil y la Junta): **sin ferry**; Makronisos
  solo con barco alquilado desde Lavrio; Gyaros solo con las visitas de la PEAEA o la plataforma
  digital Gyaros Story. En la ficha: se cuentan y se dice que no se llega sin barco propio. `baja`.
- Atenas brutalista y de la Junta (Hilton, la torre de Atenas, el Politécnico), Tesalónica (el Muro
  Blanco), el Museo de la Guerra, Delfos en KTEL, Nafplio y el Peloponeso en bus.

**Riesgos de datos.** El tren, cada mes distinto; Meteora puede recuperar el tren; Spinalonga cierra
en invierno (los barcos).

---

## Eslovenia (si · 705) — prioridad media (pareja con Croacia)

**Entrada.** UE: DNI.

**Vuelos.** **Vueling BCN–Liubliana** en 2026 vuela varios días a la semana **hasta el 19 de
noviembre** (VY1156, 2 h 05; desde ≈ 31-70 €); en invierno, no. Alternativas: Trieste (Ryanair) y bus
de 1 h 30, o Venecia y tren-bus, o Zagreb.

**Transporte.**
- Tren Liubliana–Zagreb: 9 al día, ≈ 2 h 30, desde ≈ 10-30 €. Liubliana–Postojna: cada hora, 1 h 20,
  con bus urbano gratuito (Furman, línea 2) de la estación a la cueva. Liubliana–Bled (Lesce) en tren
  o bus.
- Bus Liubliana–Kobarid: **Nomago solo viernes y domingo**, 2 h 30 (más buses vía Tolmin o desde Nova
  Gorica); Liubliana–Cerkno y Liubliana–Idrija (bus 6, 1 h) a diario.

**Sitios circo verificados.**
- **Museo de Kobarid** (Caporetto, la Gran Guerra): 8 €, ≈ 1 h con vídeo de 20 min. El **Hospital
  partisano Franja (Cerkno) sigue cerrado por daños de tormenta y reforma**; hay exposición en el
  museo de Cerkno. En la ficha, `baja` y "cerrado".
- **Línea Rupnik**: los búnkeres yugoslavos de los 30 contra Italia. Los tres de **Zaplana** (sendero de
  3 km desde Vrhnika, se entra, gratis) y el fuerte de **Goli vrh**, el único abierto con regularidad:
  **visita guiada el primer sábado de cada mes a las 10:00, 5 € en efectivo, sin reserva**. Bus
  Liubliana–Vrhnika.
- **Mina de mercurio de Idrija** (UNESCO): el pozo de Antonio, visita de 1 h 30 con casco y abrigo,
  ≈ 14 €, bus 6 desde Liubliana.
- **Postojna**: cueva con tren, 30-33 €, visitas cada hora de 9 a 18, abierta los 365 días.
- Liubliana: Plečnik, **Metelkova** (los cuarteles austrohúngaros okupados desde 1993), el búnker de
  Ježica (sin visitas, se mira por las troneras), el Museo de Historia Contemporánea.

**Riesgos de datos.** Franja puede reabrir; Vueling en invierno no vuela; el bus de Kobarid es de fin de
semana.

---

## Croacia (hr · 191) — prioridad media (pareja con Eslovenia)

**Entrada.** UE, Schengen y euro: DNI.

**Vuelos.** BCN–Zagreb directo **Croatia Airlines y TAP, ≈ 7/semana**, 2 h; Vueling y Ryanair a Zagreb
por temporadas (verificar), y a Split, Zadar y Dubrovnik en verano. Girona–Zadar con Ryanair.

**Transporte.**
- **Nocturno Zagreb–Split** (HŽPP 821/820): 22:06 → 06:53, 8 h 44; **diario del 22 de junio al 30 de
  agosto de 2026, los domingos el resto del año**. Diurno ≈ 6 h.
- **Vukovar**: 13 conexiones al día con Zagreb, ≈ 5-6 h con cambio en Vinkovci, desde ≈ 19 €. En verano
  (27 de junio–29 de agosto de 2026) nocturno Vukovar–Osijek–Zagreb–Split.
- Liubliana–Zagreb 2 h 30 en tren (ver arriba). Buses a todo (FlixBus, Arriva).

**Sitios circo verificados.**
- **Goli Otok**: el gulag de Tito en una isla; **barcos desde Rab** de 9 h (Zavratnica, Goli Otok con
  tiempo libre, Sveti Grgur, comida a bordo) **desde ≈ 30-55 €**, solo de mayo a octubre. Rab se llega
  en bus desde Rijeka o Zagreb.
- **Vukovar**: la **torre de agua** (memorial abierto en 2020, con ascensor; entrada de pago, precios en
  vukovarskivodotoranj.hr), la **Casa Memorial de Ovčara** (a 5 km, 10-17, gratis, taxi), el hospital y
  el cementerio. Ciudad entera como sitio de la guerra de 1991.
- **Petrova Gora**: el espomenik de acero inoxidable de Bakić (1981) **en ruina, sin plan ni dinero
  desde hace 30 años, con los paneles desapareciendo**; sin transporte público (taxi desde Vojnić o
  Karlovac). En la ficha: se cuenta con `media` y se avisa.
- **Jasenovac**: el campo ustacha con la Flor de Piedra de Bogdanović; **9-17, gratis** (marzo-
  noviembre); bus desde Zagreb (Novska en tren + taxi, o bus directo con pocas frecuencias).
- **Brijuni**: la isla de Tito, barco del parque desde Fažana (**30 €** el pasajero) o excursiones desde
  15 €; bus Pula–Fažana.
- **Galeb (Rijeka)**: el yate de Tito, convertido en museo-hostal por el Museo de la Ciudad de Rijeka;
  estado de apertura al público **sin verificar** (obras de años; ha tenido incidentes en puerto).
  `baja`.
- Zagreb (bloques de Novi Zagreb, el museo de los 80, túnel Grič), Pula (arena y búnkeres austriacos),
  Željava (la base aérea subterránea en la frontera bosnia: sin bus, fuera de ficha).

**Riesgos de datos.** Galeb; Goli Otok fuera de temporada; Petrova Gora puede acabar de caer.

---

## Alemania, el Este (de · 276) — prioridad media

**Entrada.** UE: DNI.

**Vuelos.** BCN–Berlín (BER) directo: **Vueling ≈ 21/semana, Ryanair ≈ 10, easyJet ≈ 10**, 2 h 30.
Desde ≈ 40 €. Leipzig y Dresde con escala o tren desde Berlín.

**Transporte.**
- **Deutschlandticket: 63 € al mes desde enero de 2026** (todo el regional, S-Bahn, metro y bus del
  país; no ICE/IC). Se compra como suscripción mensual cancelable; vale para casi toda esta ficha.
- ICE Berlín–Leipzig 1 h 15, Berlín–Dresde 2 h, Leipzig–Chemnitz 1 h (regional). Beelitz-Heilstätten
  tiene estación propia del RE7 (45 min desde Berlín). Prora: tren a Binz y parada Prora. Wünsdorf:
  RE desde Berlín, 40 min.

**Sitios circo verificados.**
- **Beelitz-Heilstätten**: el sanatorio abandonado (Hitler convaleció en 1916, hospital soviético hasta
  1994); **pasarela por las copas (Baumkronenpfad) 17 €**, marzo y octubre 10-18, abril-septiembre 10-19,
  invierno fines de semana 10-16; los edificios **solo con visita guiada de pago** aparte.
- **Teufelsberg**: la estación de escucha de la NSA sobre la montaña de escombros; **12 €**, todos los
  días de 11:00 al atardecer.
- **Wünsdorf**: la "ciudad prohibida" del mando soviético; **Casa de los Oficiales 20 € (solo efectivo)
  con reserva por teléfono o mail**; tour 1 (búnkeres Maybach y Zeppelin) martes-viernes 14:00 y fines
  de semana 11, 13 y 15 h.
- **Prora**: el coloso KdF de 4,5 km en Rügen, hoy casi todo pisos y hotel; **Centro de Documentación**
  marzo-octubre 10-18 (febrero y noviembre hasta 16), paseos guiados gratuitos cada hora de 11 a 16;
  tours en bici de mayo a julio.
- **Stasi-Museum (Lichtenberg)**: 12 €, lunes-viernes 10-18, fines de semana 11-18 (el despacho de
  Mielke intacto). Hohenschönhausen (la prisión) con visita guiada.
- **Chemnitz**: el Nischel, la cabeza de Marx de Kerbel (1971, 7,10 m, 40 t), en la calle; la ciudad fue
  Capital Europea de la Cultura 2025.
- **Eisenhüttenstadt**: la primera ciudad socialista de la RDA (Stalinstadt) con el **Museo Utopía y
  Vida Cotidiana** (170.000 objetos de la RDA); regional desde Berlín 1 h 15.
- **Spreepark**: el parque de atracciones abandonado, en obras de reapertura como parque de arte;
  visitas guiadas puntuales (precio 2026 sin verificar). Ferropolis (las excavadoras gigantes) con tren a
  Gräfenhainichen. Leipzig (Völkerschlachtdenkmal, la Runde Ecke de la Stasi).

**Riesgos de datos.** Spreepark cambia cada temporada; Beelitz tiene precios distintos por tour;
Wünsdorf pide reserva.

---

## Finlandia (fi · 246) — prioridad baja (caro)

**Entrada.** UE: DNI.

**Vuelos.** BCN–Helsinki directo **Finnair 7/semana y Norwegian 5/semana**, 4 h. Desde ≈ 60-120 €.

**Transporte.**
- **VR** con billetes dinámicos: Helsinki–Tampere desde 6,90 €; **Santa Claus Express** (nocturno
  Helsinki–Rovaniemi, 12 h, dos pisos): cabina desde ≈ 49-89 € según demanda, con ducha desde ≈ 74 €,
  reservable con meses; en Navidad se agota.
- **Frontera con Rusia cerrada** a pasajeros desde diciembre de 2023, prorrogada sin fecha (junio de
  2026); Rusia cerró además los pasos ferroviarios el 1 de julio de 2026. Karelia se ve desde este lado.
- HSL en Helsinki: billete AB 3,30 € (80 min) que incluye el **ferry a Suomenlinna** (15 min).

**Sitios circo verificados.**
- **Suomenlinna**: la fortaleza-isla sueca-rusa con búnkeres, el submarino Vesikko y el museo; ferry de
  HSL.
- **Museo de la Línea Salpa (Miehikkälä)**: los 1.200 km de búnkeres de 1940-44 contra la URSS; la
  parada de bus más cercana está a 3 km (Miehikkälä) o 10 km (Virojoki): **difícil sin coche**, se avisa.
- **Museo del Frente de Hanko**: la línea del frente de 1941 en la península que la URSS arrendó;
  10 €, junio-agosto martes-domingo 11-18 y fuera de temporada un día a la semana; tren a Hanko.
- **Sanatorio de Paimio** (Aalto, 1933): visitas guiadas 20 €; bus desde Turku.
- **Castillo de nieve de Kemi**: enero-marzo, 15 € (2024); tren Helsinki–Kemi en la línea de Rovaniemi.
  **Arktikum (Rovaniemi)**: 30 €.
- Helsinki brutalista (Merihaka, la iglesia de la roca, Itäkeskus), el búnker de la Guerra Fría bajo el
  parque de Kaisaniemi, Tampere (Lenin Museum: el único museo de Lenin fuera de Rusia, cerrado en 2024 y
  reabierto como Museo Nootti: **verificar**), el ferry a Tallin para encadenar con Estonia.

**Riesgos de datos.** Precios de VR dinámicos; el museo de Lenin de Tampere cambió de nombre y enfoque
en 2024; el más caro del radar (90-120 €/día).

---

## Orden sugerido para escribir (tercer radar)
1. **Chipre + Grecia** como pareja mediterránea (DNI, directos diarios, Varosha y Spinalonga como
   ganchos). 2. **Eslovenia + Croacia** como pareja (tren Liubliana–Zagreb, Goli Otok y la línea Rupnik).
3. **Alemania del Este** (Deutschlandticket, todo en regional). 4. **Finlandia** (cuando se acepte que
es caro).
