    ...
    <div className="space-y-12">
      <section className="relative overflow-hidden rounded-sharp border border-ink-700 bg-ink-900/40 px-6 py-12 sm:px-10 sm:py-16">
        <div
          ...
          style={{
            background:
              "radial-gradient(60% 80% at 80% 20%, rgba(232,121,249,.14), transparent 60%), radial-gradient(50% 60% at 10% 90%, rgba(34,211,238,.12), transparent 60%)",
          }}
        />
        <img
          src={BRAND.fullSmall}
          ...
          width={320}
          height={318}
          className="mb-4 w-28 select-none drop-shadow-[0_0_18px_rgba(232,121,249,.35)] lg:hidden"
          draggable={false}
        />
        {/* Texto y cráneo en la misma fila: el cráneo se centra respecto al texto, no respecto a la fila de cifras de abajo. */}
        <div className="relative flex flex-col gap-8 lg:flex-row lg:items-center lg:justify-between lg:gap-12">
          <div className="max-w-3xl">
            <div className="label-stencil flex items-center gap-2 text-neon-cyan/90">
              <Radar size={13} className="animate-pulse-glow" /> Archivo de
              destinos · edición {now.getFullYear()}
            </div>
            ...
            <p className="mt-5 max-w-2xl text-lg text-concrete-200">
              Un comparador de países para nuestra forma de viajar: muchos días,{" "}
              <strong className="text-concrete-50">sin coche</strong>, saliendo
              de Barcelona, y con debilidad por búnkeres, reliquias soviéticas,
              catacumbas, festivales con demonios y sitios que no salen en el
              top 10 de nadie.
            </p>
            <p className="mt-3 max-w-2xl text-base text-concrete-400">
              Abres un país y en un minuto sabes si mola, cuántos días pide,
              cuándo ir, cuánto cuesta y si se puede recorrer sin alquilar un
              Lada.
            </p>
          ...
          <img
            ...
            width={640}
            height={636}
            className="pointer-events-none hidden w-80 shrink-0 select-none drop-shadow-[0_0_28px_rgba(232,121,249,.35)] lg:block xl:w-96"
            draggable={false}
          />
        </div>
        <div className="relative mt-10 grid grid-cols-2 gap-3 sm:grid-cols-4">
          <Stat
            label="Países con ficha"
          ...
          <Stat
            ...
            value={places}
            accent="magenta"
            hint="raros, turbios o gigantes"
          />
          <Stat
            label="Festivales locos"
      ...
      <section className="space-y-4">
        <SectionHeader
          title="Los que más te pegan"
          kicker="Ordenados por Duke Score"
        >
          <Link
            to="/paises"
            className="text-sm text-concrete-400 hover:text-neon-cyan"
          >
            Ver todos →
          </Link>
        </SectionHeader>
      ...
      {upcoming.length > 0 && (
        <section className="space-y-4">
          <SectionHeader
            title="Festivales que caen pronto"
            kicker={`De ${monthName(thisMonth)} a ${monthName(((thisMonth + 1) % 12) + 1)}`}
          >
            <Link
              to="/festivales"
              className="flex items-center gap-1 text-sm text-concrete-400 hover:text-neon-cyan"
            >
      ...
      <section className="space-y-4">
        <SectionHeader
          title="En el radar"
          kicker="Pintan bien, pero aún no tienen ficha"
        />
          ...
          {CANDIDATES.map((k) => (
            <Panel key={k.id} className="p-3">
              <div className="flex items-center gap-2">
                ...
                <span className="text-base font-semibold text-concrete-100">
                  {k.name}
                </span>
                <span className="ml-auto flex gap-1">
                  {k.hooks.map((h) => (
                    <span
                      key={h}
                      title={
                        CATEGORY_META[h as keyof typeof CATEGORY_META]?.label
                      }
                      aria-hidden
                    >
        ...
        <p className="text-sm text-concrete-500">
          Sin login, sin backend, sin folletos. Datos curados a mano y con fecha
