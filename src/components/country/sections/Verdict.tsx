import { LIGHT_META, monthName } from "@/lib/constants";
...
export function ProsConsSection({ d }: { d: CountryDetail }) {
  return (
    <section className="space-y-4">
      <SectionHeader id="pros-contras" title="Pros y contras para mí" kicker="17 · Nada genérico" />
      <div className="grid gap-4 md:grid-cols-2">
        <Panel className="p-5">
          <div className="label-stencil mb-2 text-neon-lime">Pros</div>
          <ul className="space-y-2 text-sm text-concrete-200">
        ...
        <Panel className="p-5">
          <div className="label-stencil mb-2 text-red-400">Contras</div>
          <ul className="space-y-2 text-sm text-concrete-200">
...
export function VerdictSection({ c, d }: { c: ScoredCountry; d: CountryDetail }) {
  ...
  const logistica = fmtScore(10 - c.noCar.value * 0.6 - c.transport.value * 0.4);
  return (
    <section className="space-y-4">
      <SectionHeader id="veredicto" title="¿Te pegaría este país?" kicker="18 · Veredicto final" />
      <Panel className="panel-neon p-6">
        <div className="grid gap-6 lg:grid-cols-[auto_1fr]">
          <div className="flex flex-col items-center gap-3">
            ...
            {disagree && (
              <div className="text-center text-[11px] text-concrete-400">
                Criterio manual del autor:
                <div className="mt-1">
          ...
          <div>
            <p className="text-base leading-relaxed text-concrete-100">{d.verdict.text}</p>
            <dl className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-3">
              <V k="Duración recomendada" v={`${c.days.recommended[0]}-${c.days.recommended[1]} días`} hint={`ideal ${c.days.ideal}`} />
              <V k="Mejor mes" v={best[0] ?? "—"} hint={best.slice(1, 3).join(", ")} />
              <V k="Presupuesto" v={`${fmtEur(c.summary.inputs.cost.daily.normal)}/día`} hint={`coste ${fmtScore(c.cost.value)}/10`} />
              <V k="Dificultad logística" v={`${logistica} / 10`} hint="10 = circo logístico" />
              <V k="Circo Score" v={`${fmtScore(c.circo.value)} / 10`} />
              <V k="Viabilidad sin coche" v={`${light.emoji} ${fmtScore(c.noCar.value)}`} hint={light.label} />
            </dl>
            <div className="mt-4 font-mono text-xs text-concrete-500">
...
function V({ k, v, hint }: { k: string; v: string; hint?: string }) {
  return (
    <div className="rounded-sharp border border-ink-700/70 bg-ink-900/50 px-3 py-2">
      ...
      <dd className="mt-0.5 text-sm font-semibold text-concrete-100">{v}</dd>
